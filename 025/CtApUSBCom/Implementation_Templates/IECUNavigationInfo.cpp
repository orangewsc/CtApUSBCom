...
//-----------------Print-----------------Print-----------------Print-----------------Print-----------------Print
	...
	if (g_USBCom_LCM_DEBUG == 1)
	{
		...
		USBComMsgLCM.IECUNavigationInfo_RTE_Recv.IECUTimeStamp 				= (int64_t)(IECUTimestampRte.DeTimestampHigh << 32) | 
...
//-----------------Print-----------------Print-----------------Print-----------------Print-----------------Print
		...
		if (g_USBCom_LCM_DEBUG == 1)
		{
			ClockSourceRte = 1;
			Rte_Call_CtApUSBCom_PpPFServer_TS_GetEgtTimestamp(&IECUTimestampRte, &ClockSourceRte);
			USBComMsgLCM.IECUNavigationInfo_SOCK_Send.IECUTimeStamp = (int64_t)(IECUTimestampRte.DeTimestampHigh << 32) | 
																	  (int64_t)IECUTimestampRte.DeTimestampLow;
			for (uint8_t i = 0; i < 4; i++)
			{
